...
public class ContribuinteRepository(ContribuintesDbContext context) : IContribuinteRepository
{
    ...
    public async Task<IReadOnlyList<Contribuinte>> ListarPensionistasAsync(CancellationToken ct = default) =>
        await context.Contribuintes
            .Where(c => c.Perfil == PerfilContribuinte.Pensionista)
            .OrderBy(c => c.Nome)
            .ToListAsync(ct);

    public async Task<Contribuinte?> ObterPensionistaPorNuitAsync(Nuit nuit, CancellationToken ct = default) =>
        await context.Contribuintes
            .FirstOrDefaultAsync(c => c.Nuit == nuit && c.Perfil == PerfilContribuinte.Pensionista, ct);

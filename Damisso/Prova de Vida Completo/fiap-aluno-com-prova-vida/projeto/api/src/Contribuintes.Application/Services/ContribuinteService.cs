...
public class ContribuinteService(IContribuinteRepository repository) : IContribuinteService
{
    ...
    public async Task<IReadOnlyList<PensionistaResumoDto>> ListarPensionistasAsync(CancellationToken ct = default)
    {
        var lista = await repository.ListarPensionistasAsync(ct);
        return lista.Select(ContribuinteMapper.ParaPensionistaResumo).ToList();
    }

    public async Task<PensionistaResumoDto?> ObterPensionistaPorNuitAsync(string nuit, CancellationToken ct = default)
    {
        var entity = await repository.ObterPensionistaPorNuitAsync(Nuit.Criar(nuit), ct);
        return entity is null ? null : ContribuinteMapper.ParaPensionistaResumo(entity);
    }
}
